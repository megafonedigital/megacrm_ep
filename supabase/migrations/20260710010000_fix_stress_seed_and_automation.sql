-- Corrige seed_stress_contacts:
-- 1) contacts usa índice único PARCIAL (brand_id, wa_id) WHERE wa_id IS NOT NULL
--    (migration 20260630162755) — ON CONFLICT precisa repetir o predicado.
-- 2) Cria também a automação de teste "Stress Test — tag add/remove"
--    (trigger manual → adiciona tag stress-run → remove tag stress-run),
--    sem nenhuma dependência de WhatsApp — só exercita a esteira do broadcast.
...
BEGIN
  ...
  -- Automação de teste (idempotente por nome na marca)
  ...
  IF v_automation_id IS NULL THEN
    INSERT INTO public.automations (brand_id, name, description, status, graph, created_by)
    VALUES (
      _brand_id,
