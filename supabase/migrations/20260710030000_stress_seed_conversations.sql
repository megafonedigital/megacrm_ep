-- O automation-engine exige conversation_id; contatos fake sem conversa
-- faziam o worker receber 400 e re-tentar para sempre. O seed agora cria
-- uma conversa por contato fake (o prefetch do worker busca a conversa
-- mais recente no banco, então a fila atual destrava sozinha).
...
BEGIN
  ...
  WITH conv_ins AS (
    INSERT INTO public.conversations (brand_id, contact_id, team_id, status)
    SELECT c.brand_id, c.id, b.default_team_id, 'aberto'
      FROM public.contacts c
