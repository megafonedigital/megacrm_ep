...
BEGIN
  ...
  refilled AS (
    UPDATE public.broadcast_rate_state s
       SET tokens = LEAST(
             -- Burst cap = rate/15 (~4s do rate). Combinado com 6 mini-passes
             -- por drain espaçados em ~1.2s, dá throughput linear no alvo.
             GREATEST(b.rate_per_minute, 1)::numeric / 15.0,
...
REVOKE ALL ON FUNCTION public.claim_broadcast_dispatch_queue(integer) FROM PUBLIC, anon, authenticated;
