...
ALTER TABLE public.broadcast_dispatch_queue
  ADD COLUMN IF NOT EXISTS scheduled_send_at timestamptz NOT NULL DEFAULT now();

-- 2) Partial index for fast drain lookup
CREATE INDEX IF NOT EXISTS idx_broadcast_dispatch_queue_pending_schedule
  ON public.broadcast_dispatch_queue (broadcast_id, scheduled_send_at)
  WHERE status = 'pending';

-- 3) Rewrite enqueue_broadcast_dispatches: enqueue ALL pending with deterministic schedule
CREATE OR REPLACE FUNCTION public.enqueue_broadcast_dispatches(_broadcast_id uuid, _limit integer DEFAULT NULL)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ms entre cada envio (uniforme)
  v_interval_ms := 60000.0 / v_rate;

  -- base = maior entre now() e maior scheduled_send_at já existente para esse broadcast (pending)
  SELECT GREATEST(
           now(),
...
-- 4) Rewrite claim_broadcast_dispatch_queue: respect scheduled_send_at
