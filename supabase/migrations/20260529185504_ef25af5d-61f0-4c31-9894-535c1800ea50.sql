...
BEGIN
  ...
  -- Default seguro: nunca enfileira mais que 500 por tick. Reconcile/ticks
  -- futuros varrem o restante. Evita UPSERT em milhares por execução.
  v_effective_limit := CASE
    WHEN _limit IS NULL OR _limit <= 0 THEN 500
    ELSE _limit
  END;

  -- Calcula v_base corretamente em duas etapas para tratar o MAX NULL.
  -- Bug anterior: LEAST(NULL, X) = X em Postgres (NULL é ignorado), então
  -- na primeira passagem o COALESCE nunca caía em now() e v_base ficava
  -- fixado em now() + 2 min, criando atraso forçado na largada.
  ...
  IF v_max_sched IS NULL THEN
    v_base := now();  -- primeiro tick / sem pendentes em janela
  ELSE
    v_base := GREATEST(
      now(),
      LEAST(
