...
BEGIN
  ...
  refilled AS (
    UPDATE public.broadcast_rate_state s
       SET tokens = LEAST(
             -- Burst cap = rate/12 (1 ciclo de cron de 5s). Garante curva
             -- linear: cada drain consome exatamente as fichas acumuladas.
             GREATEST(1, ROUND(GREATEST(b.rate_per_minute, 1)::numeric / 12.0)),
  ...
  SELECT u.id, u.broadcast_id, u.target_id, u.brand_id, u.automation_id,
    ...
    FROM updated u
    -- força materialização do consumed CTE
