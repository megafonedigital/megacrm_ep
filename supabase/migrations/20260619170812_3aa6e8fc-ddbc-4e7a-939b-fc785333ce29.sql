...
-- A) Leaky-bucket per-broadcast no claim
...
BEGIN
  RETURN QUERY
  WITH
  -- Budget restante por broadcast (rate_per_minute - dispatched nos últimos 60s).
  budgets AS (
    SELECT b.id AS broadcast_id,
           GREATEST(
             0,
             GREATEST(1, b.rate_per_minute) - COALESCE((
               SELECT count(*)
                 FROM public.broadcast_dispatch_queue q2
                WHERE q2.broadcast_id = b.id
                  AND q2.dispatched_at >= now() - interval '60 seconds'
             ), 0)
             -- desconta itens já em processamento (claimed mas ainda não finalizados)
             - COALESCE((
               SELECT count(*)
                 FROM public.broadcast_dispatch_queue q3
                WHERE q3.broadcast_id = b.id
                  AND q3.status = 'processing'
                  AND q3.claimed_at >= now() - interval '60 seconds'
             ), 0)
           )::int AS budget_remaining
      FROM public.broadcasts b
     WHERE b.status = 'running'
  ),
  -- Candidatos elegíveis com ranking por broadcast, já cortados pelo budget.
  eligible AS (
    SELECT q.id AS queue_id,
      ...
      JOIN budgets bd ON bd.broadcast_id = q.broadcast_id
     WHERE q.status = 'pending'
       AND q.next_attempt_at <= now()
       AND q.scheduled_send_at <= now()
       AND bd.budget_remaining > 0
  ),
  capped AS (
    SELECT e.queue_id
      FROM eligible e
      JOIN budgets bd ON bd.broadcast_id = e.broadcast_id
     WHERE e.rn <= bd.budget_remaining
...
-- B) Pacing no enqueue quando fila está vazia: começa exatamente em now()
--    em vez de now() - 5s, preservando o espaçamento desde o primeiro item.
...
BEGIN
  ...
  IF v_max_sched IS NULL THEN
    -- FIX B: começa exatamente em now() (não now()-5s). Garante que o
    -- espaçamento (v_interval_ms) seja respeitado a partir do primeiro item,
    -- eliminando o mini-burst em cold-start / fila vazia entre ticks.
    v_base := now();
  ELSE
    -- Continuação de série já existente: mantém o mínimo de now()-5s para
    -- não atrasar quando a fila esvaziou um pouco entre ticks.
    v_base := GREATEST(
