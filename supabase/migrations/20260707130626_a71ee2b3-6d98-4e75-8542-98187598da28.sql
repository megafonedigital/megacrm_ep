-- MUDANÇA 2: enqueue_broadcast_dispatches — carry-ahead 90s → 20s (resto preservado)
...
DECLARE
  ...
  v_max_carry_ahead interval := interval '20 seconds';
...
-- MUDANÇA 3: claim_broadcast_dispatch_queue — cap do bucket rate/30 → rate/15 (resto preservado)
...
BEGIN
  ...
  refilled AS (
    UPDATE public.broadcast_rate_state s
       SET tokens = LEAST(
             GREATEST(5, ROUND(GREATEST(b.rate_per_minute, 1)::numeric / 15.0)),
...
-- MUDANÇA 4: índice para o LATERAL de conversations no enqueue
CREATE INDEX IF NOT EXISTS idx_conversations_brand_contact_created
  ON public.conversations (brand_id, contact_id, created_at DESC);
