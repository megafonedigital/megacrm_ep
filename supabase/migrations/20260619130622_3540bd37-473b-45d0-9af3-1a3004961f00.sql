...
-- 1) Corrige a função de enfileiramento
...
BEGIN
  ...
  -- Considera TODOS os itens já agendados no futuro, não apenas os próximos 2 min.
  -- O bug anterior: quando o lote anterior agendava itens além de now()+2min,
  -- esta query retornava NULL e a função re-ancorava em started_at, sobrepondo
  -- novos disparos por cima dos já agendados (rajadas e estouro do rate).
  ...
  -- Throttle: se já tem fila suficiente à frente, não enfileira mais.
  -- O próximo tick volta a chamar quando a fila drenar.
...
-- 2) Reespalha os itens pending do broadcast em andamento (Captura API 2 - Janela)
-- Para qualquer broadcast running, recalcula scheduled_send_at uniformemente
-- a partir de now() respeitando rate_per_minute, preservando a ordem.
WITH b AS (
  SELECT id, GREATEST(1, rate_per_minute) AS rate
    FROM public.broadcasts
   WHERE id = '2e23d96d-2daa-4169-9add-cd32133f780b'
     AND status = 'running'
),
ranked AS (
  SELECT q.id,
         b.rate,
         row_number() OVER (ORDER BY q.scheduled_send_at, q.created_at) AS rn
    FROM public.broadcast_dispatch_queue q
    JOIN b ON b.id = q.broadcast_id
   WHERE q.status = 'pending'
)
UPDATE public.broadcast_dispatch_queue q
   SET scheduled_send_at = now() + make_interval(secs => ((r.rn - 1) * (60.0 / r.rate))),
       next_attempt_at = now() + make_interval(secs => ((r.rn - 1) * (60.0 / r.rate))),
       updated_at = now()
  FROM ranked r
 WHERE q.id = r.id;
